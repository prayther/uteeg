...
curl -f http://"${GATEWAY}"/ks/katello-export/redhat-Default_Organization_View-v1.0/redhat/Library/ && hammer organization update --name ${ORG} --redhat-repository-url http://"${GATEWAY}"/ks/katello-export/redhat-Default_Organization_View-v1.0/redhat/Library/
...
repolist () {for i in $(hammer --csv repository list --organization=${ORG} | grep -i "7 Server" | grep -i optional | awk -F, {'print $1'} | grep -vi '^ID')
