...
for LOC in ${LOC_var};do
  for ORG in ${ORG_var};do
	...
	for NET in ${NET_var};do
          hammer hostgroup create --architecture="x86_64" --organization "${ORG}" --locations "${LOC}" --domain="${DOMAIN}" --medium-id="${MEDID}" --name="HG_${LE}_${CCV}_${ORG}_${LOC}" --subnet="${NET}" --partition-table-id="${PARTID}" --operatingsystem-id="${OSID}"
          #hammer organization add-hostgroup --name ${ORG} --hostgroup "HG_${LE}_${CCV}"
        done
      done
    done
