...
# $Id: buildServer-just-iso.sh 763 2012-08-15 13:52:46Z sysutil $
...
#       ----------------------------------------------------------------
        ...
        echo "${PROGNAME}: ${1:-"Unknown Error"}" >> /var/log/sim
        exit 1
}
...
for hostfiles in `ls /srv/www/htdocs/sim/output/*`;do 
 if [ $hostfiles != "" ];then
  for vm in `cat $hostfiles |  sed -e 's/\ /_/g'`;do
    for cleanuphostfiles in `ls /srv/www/htdocs/sim/output/*`;do
