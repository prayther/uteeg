...
# $Id: sosreport.sh 1115 2012-11-21 18:38:07Z sysutil $
DIRNAME=`date +'%s'`
TIMESTAMP=`date +'%x,%R'`
#HOSTNAME=`hostname -s`
HOSTNAME=`facter hostname`
ROLE=`grep ^$HOSTNAME /etc/puppet/manifests/nodes/hosts.master.txt | awk -F, '{ print $26 }'`
ENVIRONMENTCAPS=`grep ^$HOSTNAME /etc/puppet/manifests/nodes/hosts.master.txt | awk -F, '{ print $33 }' | tr "[:lower:]" "[:upper:]"`
ENVIRONMENT=`grep ^$HOSTNAME /etc/puppet/manifests/nodes/hosts.master.txt | awk -F, '{ print $33 }'`
POC=`grep ^$HOSTNAME /etc/puppet/manifests/nodes/hosts.master.txt | awk -F, '{ print $10 }'`
SERVICENAME=`grep ^$HOSTNAME /etc/puppet/manifests/nodes/hosts.master.txt | awk -F, '{ print $47 }'`
SERVICENAMELOWER=`grep ^$HOSTNAME /etc/puppet/manifests/nodes/hosts.master.txt | awk -F, '{ print $47 }' | tr "[:upper:]" "[:lower:]"`
...
REDHATRELEASE=`facter lsbdistrelease`
ARCH=`uname -p`
#KERNELVERSION=`uname -r`
KERNELVERSION=`facter kernelrelease`
DATE=`date +"%m-%d-%y"`
SIMHOST="sim"
#DATE="04-01-13"
# DSE section
  ...
  if [ -d /opt/PostgresPlus ];then
    POSTGRESENTERPRISEDB=`su -l enterprisedb -c "cd /opt/PostgresPlus/current/bin/ && ./.edb-psql --version" | grep psql | awk -F" " '{ print $3 }'`
  fi
...
# send it over to host "163.240.36.253" to be viewed/downloaded, http://163.240.36.253/sos
su -l sysutil -c "ssh -o StrictHostKeyChecking=no 163.240.36.253 mkdir -p /var/www/html/sos/$HOSTNAME/archive/monthly"
su -l sysutil -c "ssh 163.240.36.253 rm -rf /var/www/html/sos/$HOSTNAME/$HOSTNAME"
#su -l sysutil -c "ssh 163.240.36.253 find /var/www/html/sos/$HOSTNAME/$HOSTNAME/archive -name '*.bz2' -mtime +31 -exec rm -f {} ;"
su -l sysutil -c "ssh 163.240.36.253 find /var/www/html/sos/$HOSTNAME/archive -name "*.bz2" -mtime +31 | xargs rm -f"
su -l sysutil -c "ssh 163.240.36.253 find /var/www/html/sos/$HOSTNAME/archive/monthly -name "*.bz" | xargs rm -f"
su -l sysutil -c "rsync -a /tmp/$DIRNAME/$HOSTNAME sysutil@163.240.36.253:/var/www/html/sos/$HOSTNAME/"
if [ `echo $DATE | awk -F"-" '{ print $2 }'` == "01" ];then
  su -l sysutil -c "rsync -a /tmp/$DIRNAME/*.bz2 sysutil@163.240.36.253:/var/www/html/sos/$HOSTNAME/archive/monthly"
  else
  su -l sysutil -c "rsync -a /tmp/$DIRNAME/*.bz2 sysutil@163.240.36.253:/var/www/html/sos/$HOSTNAME/archive/"
fi
