...
# Kickstart Actions:
	...
	class ac-3::p1 {
		...
		#nosuid, nodev, on /data
		exec { "sed -i 's/\( \/data.*defaults\)/\1,nosuid/' /etc/fstab":
			onlyif => "test `grep ' \/data ' /etc/fstab | grep -c nosuid` -eq 0",
		}

		#nosuid, nodev, on /home
		exec { "sed -i 's/\( \/home.*defaults\)/\1,nosuid/' /etc/fstab":
