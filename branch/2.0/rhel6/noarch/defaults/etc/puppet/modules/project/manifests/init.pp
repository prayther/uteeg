...
# $Id: init.pp 799 2012-08-22 16:04:35Z sysutil $
...
class spawar_sw1 {
  exec { 'install required packages':
    #command   => '/usr/bin/yum install -y audispd-plugins vmware-open-vm-tools nagios-plugins-nrpe nrpe uvscanspawar acl bacula-client yum-security ovaldi puppet-0.25.5 setools sysstat audit screen vim-enhanced',
    command   => '/usr/bin/yum install -y defaults ircd-ratbox-mkpasswd audispd-plugins vmware-open-vm-tools avvdatspawar uvscanspawar acl yum-security ovaldi puppet-0.25.5 setools sysstat audit screen vim-enhanced',
...
#  spawar_user_nomore { 'oolivares':,
#   }
#  spawar_user_nomore { 'rayjd':,
#   }
...
class spawar_files {
  ...
  file { '/root/HBSS/install-linux-rpm-46-1694.sh':
    ensure => present,
