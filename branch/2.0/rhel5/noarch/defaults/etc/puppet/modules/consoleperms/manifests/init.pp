...
class consoleperms {
	...
	exec {
		"Restrict ConsoleDevice Access":
			command => "/bin/sed -i 's/^<x*console>/#&/g' /etc/security/console.perms.d/50-default.perms",
			user    => "root",
			onlyif	=> "/bin/grep -E '^<x*console>' /etc/security/console.perms.d/50-default.perms",
	}
}
